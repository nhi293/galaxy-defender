// ============================================================
// Enemy.js – Ke dich va Boss (V2.2 Final)
// Boss: 4 Phase, 6 ky nang, sinh quai moi 10s, hoi mau
// Boss roi do moi khi mat 10-15% HP (nguong random)
// ============================================================
...
export default class Enemy extends Phaser.Physics.Arcade.Sprite {
    ...
    spawn(x, y, typeKey, difficulty = 1) {
        ...
        // Boss-specific drop threshold: roi do moi khi mat 10-15% HP (nguong random)
        ...
        if (this.isBoss) {
            ...
            this._laserDmg = Math.floor(this.bulletDmg * 1.4);
    ...
    hit(damage) {
        ...
        // Boss: check phase + drop item moi mat 10-15% HP
        if (this.isBoss) {
    ...
    _checkBossThresholdDrop() {
        ...
        // Roi do moi khi mat 10-15% HP (nguong random moi lan)
    ...
    fireAimed() {
        const scene = this.scene;
        if (!scene.player || !scene.player.isAlive) return;

        const dx = scene.player.x - this.x;
        const dy = scene.player.y - (this.y + 30);
        const angle = Phaser.Math.RadToDeg(Math.atan2(dy, dx));
        const count = this._rageMode ? 3 : 1;

        for (let i = 0; i < count; i++) {
            const offset = (i - (count - 1) / 2) * 10;
            const b = this.scene.enemyBullets.get();
            if (b) b.fireAngled(this.x, this.y + 30, this.bulletDmg, angle + offset, this.bulletSpeed * 1.15);
        }
    }
    ...
    preUpdate(time, delta) {
        ...
        if (this.isBoss) {
            ...
            if (!this._hasReachedPatrolY) {
                ...
                // Aimed shot (ban duoi theo nguoi choi)
                if (this.bossSkills.includes("aimed")) {
                    this._aimedCooldown -= delta;
                    if (this._aimedCooldown <= 0) {
                        this.fireAimed();
                        this._aimedCooldown = Phaser.Math.Between(2500, 4500);
                    }
                }
