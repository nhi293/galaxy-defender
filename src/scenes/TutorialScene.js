...
export default class TutorialScene extends Phaser.Scene {
    ...
    startAutoShoot() {
        // Huy timer cu truoc khi tao moi (tranh duplicate)
        if (this.shootTimer) {
            this.shootTimer.remove(false);
            this.shootTimer = null;
        }
        this.shootTimer = this.time.addEvent({
            delay: this.player?.shootDelay ?? 300,
            loop: true,
            callback: () => {
                if (this.player?.isAlive && !this._finished) this.player.shoot();
            }
        });
    }
    ...
    _finish(victory = true) {
        ...
        if (this.shootTimer) { this.shootTimer.remove(false); this.shootTimer = null; }
