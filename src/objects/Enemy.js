...
export default class Enemy extends Phaser.Physics.Arcade.Sprite {
    ...
    _summonMinions(emergency = false) {
    const scene = this.scene;
    if (!scene?.events) return;
    const count = emergency
        ? Math.max(2, Math.floor(this._summonCount * 0.6))
        : (this._rageMode ? this._summonCount + 3 : this._summonCount);

    scene.events.emit("boss-summon-tracked", {
        boss: this, count, type: this._summonType, emergency
    });
}
